  ...
  <form name="frmsearch" method="post" action="issuedbookdisplay.php">
    ...
    if(isset($_POST["txtsearch"]))
    {
        ...
        <td><b>Book Issued  Date</b></td>
          ...
          $data=$con->query("select admission.sname,admission.fname,admission.s_mobile_no,admission.regno,tblbookissue.bookisbn,tblbookissue.issuedate,tblbookissue.returnstatus from admission  inner join tblbookissue on admission.regno=tblbookissue.regno where tblissuebook.returnstatus!='NULL' admission.regno='$search' or bookisbn like '%$search%' or issuedate!='-' ");
